...
def calculate_daily_dynamodb_attribution_by_tenant(event, context):
    ...
    # Get total cost of DynamoDB for the current date
    ...
    # Get DynamoDB usage by tenant using filter_log_events API.
    ...
    for tenant_id, usage in tenant_usage.items():
        ...
        # Save the tenant attribution data inside a dynamodb table
        try:
...
# Below function considers number of invocations as the metric to calculate usage and cost.
# You can go more granular by recording duration of each invocation. Since our functions
# are basic CRUD this invocation count works as a ballpark cost estimate.
def calculate_daily_lambda_attribution_by_tenant(event, context):
    ...
    # Use filter_log_events API instead of Logs Insights for accurate counting.
    # Logs Insights has an indexing limitation where cold start logs may not be indexed.
    # Get Lambda invocations by tenant using filter_log_events API.
...
def __get_total_service_cost(servicename, start_date_time, end_date_time):
    # Ignoring startTime and endTime filter because we are using a static/sample CUR file
    ...
    total_cost = result['ResultSet']['Rows'][1]['Data'][0]['VarCharValue']
    print(total_cost)
...
def __filter_log_events_with_pattern(logs_client, log_group_name, filter_pattern, start_time, end_time):
    """Use filter_log_events API for accurate log counting.

    Logs Insights has an indexing limitation where the first batch of log events
    in a new log stream (cold start logs) may not be indexed. filter_log_events
    returns ALL matching log events reliably.
    """
    events = []
    ...
    for log_group_name in log_group_names:
        ...
        for event in events:
            try:
                ...
                tenant_id = log_data.get('tenant_id', 'unknown')
                consumed_rcu = Decimal(str(log_data.get('ReadCapacityUnits', 0)))
                consumed_wcu = Decimal(str(log_data.get('WriteCapacityUnits', 0)))
...
def __get_list_of_log_group_names():
    ...
    # Discover the Lab 4 tenant stack (orchestration nested stack pattern).
                ...
                for resource in stack_resources['StackResourceSummaries']:
                    if resource["LogicalResourceId"] in ("CreateProductFunction", "UpdateProductFunction", "GetProductsFunction"):
                        __add_log_group_name(
                            logs,
                            ''.join([log_group_prefix, resource["PhysicalResourceId"]]),
                            log_group_names
                        )
...
def __get_start_date_time():
    ...
    start_date_time = int(datetime.now(tz=time_zone).date().strftime('%s'))
    return start_date_time


def __get_end_date_time():
    time_zone = datetime.now().astimezone().tzinfo
    end_date_time = int((datetime.now(tz=time_zone) + timedelta(days=1)).date().strftime('%s'))
