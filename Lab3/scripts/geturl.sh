...
print_usage() {
    ...
    echo "  $0 --stack-name serverless-saas-lab3            # Get URLs for specific lab"
    echo "  $0 --profile my-profile                         # Get URLs with specific profile"
...
print_message "$BLUE" "=========================================="
print_message "$BLUE" "Lab3 Application URLs"
print_message "$BLUE" "=========================================="
echo ""

if [[ -n "$AWS_PROFILE" ]]; then
    PREPROVISIONED_ADMIN_SITE=$(aws cloudformation list-exports --profile "$AWS_PROFILE" --query "Exports[?Name=='Serverless-SaaS-AdminAppSite'].Value" --output text --region "$AWS_REGION" 2>/dev/null || echo "")
else
...
if [ ! -z "$PREPROVISIONED_ADMIN_SITE" ]; then
  ...
  if [[ -n "$AWS_PROFILE" ]]; then
    ADMIN_SITE_URL=$(aws cloudformation list-exports --profile "$AWS_PROFILE" --query "Exports[?Name=='Serverless-SaaS-AdminAppSite'].Value" --output text --region "$AWS_REGION" 2>/dev/null || echo "")
    LANDING_APP_SITE_URL=$(aws cloudformation list-exports --profile "$AWS_PROFILE" --query "Exports[?Name=='Serverless-SaaS-LandingApplicationSite'].Value" --output text --region "$AWS_REGION" 2>/dev/null || echo "")
    APP_SITE_URL=$(aws cloudformation list-exports --profile "$AWS_PROFILE" --query "Exports[?Name=='Serverless-SaaS-ApplicationSite'].Value" --output text --region "$AWS_REGION" 2>/dev/null || echo "")
  else
    ADMIN_SITE_URL=$(aws cloudformation list-exports --query "Exports[?Name=='Serverless-SaaS-AdminAppSite'].Value" --output text --region "$AWS_REGION" 2>/dev/null || echo "")
    LANDING_APP_SITE_URL=$(aws cloudformation list-exports --query "Exports[?Name=='Serverless-SaaS-LandingApplicationSite'].Value" --output text --region "$AWS_REGION" 2>/dev/null || echo "")
    APP_SITE_URL=$(aws cloudformation list-exports --query "Exports[?Name=='Serverless-SaaS-ApplicationSite'].Value" --output text --region "$AWS_REGION" 2>/dev/null || echo "")
  fi
  ...
  if [[ -n "$AWS_PROFILE" ]]; then
    if aws cloudformation describe-stacks --profile "$AWS_PROFILE" --stack-name "$SHARED_STACK_NAME" --region "$AWS_REGION" &>/dev/null; then
      ADMIN_SITE_URL=$(aws cloudformation describe-stacks --profile "$AWS_PROFILE" --stack-name "$SHARED_STACK_NAME" --region "$AWS_REGION" --query "Stacks[0].Outputs[?OutputKey=='AdminAppSite'].OutputValue" --output text 2>/dev/null || echo "")
      LANDING_APP_SITE_URL=$(aws cloudformation describe-stacks --profile "$AWS_PROFILE" --stack-name "$SHARED_STACK_NAME" --region "$AWS_REGION" --query "Stacks[0].Outputs[?OutputKey=='LandingApplicationSite'].OutputValue" --output text 2>/dev/null || echo "")
      APP_SITE_URL=$(aws cloudformation describe-stacks --profile "$AWS_PROFILE" --stack-name "$SHARED_STACK_NAME" --region "$AWS_REGION" --query "Stacks[0].Outputs[?OutputKey=='ApplicationSite'].OutputValue" --output text 2>/dev/null || echo "")
    else
      print_message "$RED" "Error: Stack $SHARED_STACK_NAME not found in region $AWS_REGION"
      print_message "$YELLOW" "Make sure Lab3 is deployed first with: ./deployment.sh -s"
      exit 1
    fi
  else
