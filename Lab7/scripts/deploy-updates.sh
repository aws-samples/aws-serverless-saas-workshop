...
# Functions whose code the participant modifies in Lab 7 TODOs.
declare -a FUNCTIONS_TO_UPDATE=(
    "serverless-saas-lab7-dynamodb-cost|TenantUsageAndCost"
    "serverless-saas-lab7-lambda-cost|TenantUsageAndCost"
)
...
print_message "$BLUE" "=========================================="
print_message "$BLUE" "Lab 7 — Deploy Code Updates"
print_message "$BLUE" "=========================================="
echo ""
...
if command -v pylint &> /dev/null; then
    $PYTHON_CMD -m pylint -E -d E0401,E1121 $(find "$SERVER_DIR/TenantUsageAndCost" -iname "*.py" -not -path "*/.aws-sam/*") || {
...
# Step 2: Pre-flight
print_message "$YELLOW" "Step 2: Pre-flight — checking Lab 7 deployment..."
CANARY="serverless-saas-lab7-dynamodb-cost"
if ! aws_cmd lambda get-function --function-name "$CANARY" \
        --query 'Configuration.FunctionName' --output text > /dev/null 2>&1; then
    print_message "$RED" "ERROR: Lab 7 does not appear to be deployed."
    print_message "$YELLOW" "  Could not find Lambda function: $CANARY"
    exit 1
fi
print_message "$GREEN" "✓ Lab 7 is deployed"
echo ""
...
for entry in "${FUNCTIONS_TO_UPDATE[@]}"; do
    ...
    # 3. Include layers/ (shared utilities) if present
...
print_message "$GREEN" "=========================================="
echo ""
print_message "$BLUE" "The code changes are now live. Next steps:"
print_message "$BLUE" "  1. Run ./seed-sample-data.sh <bearer-token> to generate tenant activity"
print_message "$BLUE" "  2. Wait ~5 minutes for the EventBridge Scheduler to trigger aggregation"
print_message "$BLUE" "  3. Check DynamoDB table: TenantCostAndUsageAttribution-lab7"
echo ""
