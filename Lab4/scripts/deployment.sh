...
if [[ $server -eq 1 ]] || [[ $bootstrap -eq 1 ]]; then
  ...
  # Check if S3 buckets already exist and delete them if they do
  echo "Checking for existing S3 buckets..."
  BUCKETS_TO_CHECK=(
    "serverless-saas-workshop-lab4-adminappbucket"
    "serverless-saas-workshop-lab4-appbucket"
    "serverless-saas-workshop-lab4-landingappbucket"
  )
  
  for BUCKET in "${BUCKETS_TO_CHECK[@]}"; do
    if aws s3 ls "s3://$BUCKET" 2>/dev/null; then
      echo "Found existing bucket: $BUCKET - Emptying and removing..."
...
if [[ $server -eq 1 ]] || [[ $tenant -eq 1 ]]; then
  ...
  # Wait for shared stack exports to be available
  echo "Waiting for shared stack exports to be available..."
  REGION=$(aws configure get region)
  MAX_ATTEMPTS=30
  ATTEMPT=0
  
  while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
    EXPORT_CHECK=$(aws cloudformation list-exports --region=$REGION --query "Exports[?Name=='Serverless-SaaS-CognitoTenantUserPoolId-lab4'].Value" --output text 2>&1)
    
    if [ ! -z "$EXPORT_CHECK" ] && [ "$EXPORT_CHECK" != "None" ]; then
      echo "✓ Shared stack exports are ready"
      break
    fi
    
    ATTEMPT=$((ATTEMPT+1))
    if [ $ATTEMPT -eq $MAX_ATTEMPTS ]; then
      echo "✗ Timeout waiting for shared stack exports. Please check the shared stack deployment."
      exit 1
    fi
    
    echo "Waiting for exports... (attempt $ATTEMPT/$MAX_ATTEMPTS)"
    sleep 10
  done
...
# Wait for stacks to be fully created before deploying client
if [[ $client -eq 1 ]]; then
  echo "Waiting for CloudFormation stacks to complete before deploying client..."
  
  # Check if shared stack exists and wait for it
  if aws cloudformation describe-stacks --stack-name serverless-saas-workshop-shared-lab4 &>/dev/null; then
    echo "Waiting for shared stack to complete..."
    aws cloudformation wait stack-create-complete --stack-name serverless-saas-workshop-shared-lab4 2>/dev/null || \
    aws cloudformation wait stack-update-complete --stack-name serverless-saas-workshop-shared-lab4 2>/dev/null || true
    echo "Shared stack is ready"
  fi
  
  # Check if tenant stack exists and wait for it
  if aws cloudformation describe-stacks --stack-name serverless-saas-workshop-tenant-lab4 &>/dev/null; then
    echo "Waiting for tenant stack to complete..."
    aws cloudformation wait stack-create-complete --stack-name serverless-saas-workshop-tenant-lab4 2>/dev/null || \
    aws cloudformation wait stack-update-complete --stack-name serverless-saas-workshop-tenant-lab4 2>/dev/null || true
    echo "Tenant stack is ready"
  fi
  
  echo "All stacks are ready. Proceeding with client deployment..."
fi
...
if [ "$IS_RUNNING_IN_EVENT_ENGINE" = false ]; then
  ...
  APP_SITE_BUCKET=$(aws cloudformation describe-stacks --stack-name serverless-saas-workshop-shared-lab4 --query "Stacks[0].Outputs[?OutputKey=='ApplicationSiteBucket'].OutputValue" --output text)
...
if [[ $client -eq 1 ]]; then
  ...
  ADMIN_APIGATEWAYURL=$(aws cloudformation describe-stacks --stack-name serverless-saas-workshop-shared-lab4 --query "Stacks[0].Outputs[?OutputKey=='AdminApi'].OutputValue" --output text)
  APP_APIGATEWAYURL=$(aws cloudformation describe-stacks --stack-name serverless-saas-workshop-tenant-lab4 --query "Stacks[0].Outputs[?OutputKey=='TenantAPI'].OutputValue" --output text)
  APP_APPCLIENTID=$(aws cloudformation describe-stacks --stack-name serverless-saas-workshop-shared-lab4 --query "Stacks[0].Outputs[?OutputKey=='CognitoTenantAppClientId'].OutputValue" --output text)
  APP_USERPOOLID=$(aws cloudformation describe-stacks --stack-name serverless-saas-workshop-shared-lab4 --query "Stacks[0].Outputs[?OutputKey=='CognitoTenantUserPoolId'].OutputValue" --output text)
  ADMIN_SITE_BUCKET=$(aws cloudformation describe-stacks --stack-name serverless-saas-workshop-shared-lab4 --query "Stacks[0].Outputs[?OutputKey=='AdminSiteBucket'].OutputValue" --output text)
  LANDING_APP_SITE_BUCKET=$(aws cloudformation describe-stacks --stack-name serverless-saas-workshop-shared-lab4 --query "Stacks[0].Outputs[?OutputKey=='LandingApplicationSiteBucket'].OutputValue" --output text)
...
EoF
  ...
  echo "Successfully completed deploying Landing UI"

  # Configuring App UI
  echo "Configuring App UI"
  cd ../Application
...
if [[ $server -eq 1 ]] && [ ! -z "$TENANT_ADMIN_EMAIL" ]; then
  ...
  ADMIN_API_URL=$(aws cloudformation describe-stacks --stack-name serverless-saas-workshop-shared-lab4 --query "Stacks[0].Outputs[?OutputKey=='AdminApi'].OutputValue" --output text 2>/dev/null)
